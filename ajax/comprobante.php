...
else
{
	...
	if ($_SESSION[$permisovista]==1)
	{
		...
		$idimpuesto=isset($_POST["impuesto"])? limpiarCadena($_POST["impuesto"]):"";
		$idusuariocambio = $_SESSION['idusuario'];

		switch ($_GET["op"]){
			case 'guardaryeditar':
				if (empty($idcomprobante)){
					$rspta=$comprobante->insertar($nombre,$descripcion,$idimpuesto,$idusuariocambio);
					echo $rspta ? 0 : 1;
				}
				else {
					$rspta=$comprobante->editar($idcomprobante,$nombre,$descripcion,$idimpuesto,$idusuariocambio);
					echo $rspta ? 2 : 3;
				}
			break;
			...
			case 'listar':
				$rspta=$comprobante->listar();
		 		...
		 		while ($reg=$rspta->fetch_object()){
		 			$data[]=array(
		 				"0"=>($reg->condicion)?'<button class="btn btn-xs btn-warning" onclick="mostrar('.$reg->idcomprobante.')"><i class="fa fa-edit"></i></button>'.
		 					' <button class="btn btn-xs btn-danger" onclick="desactivar('.$reg->idcomprobante.','.$idusuariocambio.')"><i class="fa fa-close"></i></button>':
		 					'<button class="btn btn-xs btn-warning" onclick="mostrar('.$reg->idcomprobante.')"><i class="fa fa-eye"></i></button>'.
		 					' <button class="btn  btn-xs btn-primary" onclick="activar('.$reg->idcomprobante.','.$idusuariocambio.')"><i class="fa fa-check"></i></button>',
		 				"1"=>$reg->nombre,
		 				"2"=>$reg->descripcion,
